...
function rmKnock(h,s,i){if(h.gone)return;h.gone=true;const m=h.mesh,spd=Math.hypot(s.vx,s.vz);
  ...
  RM.timers.push(setTimeout(()=>{if(!RM.on)return;
    ...
    h.gone=false},9000));
  RM.knocks[i]++;rmSaveDesert();rmFeature(i,performance.now());sndThud();
...
const RM_PLACES=['diner','gas station','town hairpin','back road','far hairpin','turn two','infield','pits'];
...
function rmCheck(c,s,i,now){const set=RM.checks[i];if(set.has(c.k))return;set.add(c.k);rmSaveDesert();
